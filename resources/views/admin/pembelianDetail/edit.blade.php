...
@section('content')
    ...
    <hr>
    <div class="card">
        <form action="{{ route('admin.pembelianDetail.update', $pembelianDetail->id) }}" method="post"
            ...
            <div class="card-body">
                    ...
                    <div class="mb-3 col">
                        ...
                        <select name="rak_id" id="" class="form-select form-select-sm mb-3 select2" required>
                            @foreach ($rak as $d)
                                ...
                                <option value="{{ $d->id }}"
                                    {{ $pembelianDetail->rak_id == $d->id ? 'selected' : '' }}>{{ $d->kodeLokasi }} -
                                    {{ $d->gudang }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row">
                    <div class="mb-3 col">
                        ...
                        <input class="form-control form-control-sm mb-3" type="number" name="hargaBeli"
                            value="{{ $pembelianDetail->hargaBeli }}" placeholder="Harga Beli/Pcs"
                    ...
                    <div class="mb-3 col">
                        ...
                        <input class="form-control form-control-sm mb-3" type="number" name="jumlahSj"
                            value="{{ $pembelianDetail->jumlahSj }}" placeholder="Quantity"
                            aria-label="default input example" required>
                    </div>
                    <div class="mb-3 col">
                        <label for="formFile" class="form-label">Harga Jual/Pcs (+ 20% dari harga beli)</label>
                        <input class="form-control form-control-sm mb-3" type="number" name="hargaJual"
                            value="{{ $pembelianDetail->sparepart->stok->hargaJual }}" placeholder="Harga Jual/Pcs"
                            aria-label="default input example" required>
                    </div>
                </div>
            </div>
            ...
            <div class="card-footer text-end">
                <button type="submit" class="btn btn-primary px-3 radius-30"><i class="bi bi-save2-fill"></i>
                    Simpan</button>
                <a href="{{ route('admin.pembelian.show', $pembelianDetail->pembelian_id) }}"
